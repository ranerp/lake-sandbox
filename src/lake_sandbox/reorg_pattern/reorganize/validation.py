...
def validate_parquet_file(file_path: Path, conn: duckdb.DuckDBPyConnection) -> Tuple[bool, int, Optional[str]]:
...
def check_existing_chunk(chunk_file: Path, 
    ...
    if error == "File does not exist":
        ...
        typer.echo(f"  ⚠ Existing chunk {chunk_id} is corrupted ({error}), recreating...")
