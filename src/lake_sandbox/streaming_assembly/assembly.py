...
def streaming_assembly(
    ...
    output_path: str = typer.Option(
        "./output", "--output", "-o", help="Output path for assembled data"
    ),
    batch_size: int = typer.Option(
        1000, "--batch-size", help="Batch size for streaming processing"
    ),
    format: str = typer.Option(
        "parquet", "--format", help="Output format (parquet, json, csv)"
    ),
) -> None:
