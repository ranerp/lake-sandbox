...
def get_random_time_window(conn: duckdb.DuckDBPyConnection, 
    ...
    if not start_date or not end_date:
        raise ValueError(f"Could not select a {window_days}-day window from available data")
...
def get_random_parcels_in_window(conn: duckdb.DuckDBPyConnection,
    ...
    if df.empty:
        raise ValueError(f"No data found for the time window {start_date} to {end_date}")
...
def display_sample_statistics(stats: dict, console: Console):
    ...
    table = Table(title="Sample Statistics", show_header=True, header_style="bold magenta")
    ...
    table.add_row("NDVI Range", f"{stats['ndvi_range'][0]:.3f} - {stats['ndvi_range'][1]:.3f}")
    table.add_row("Temperature Range", f"{stats['temp_range'][0]:.1f}°C - {stats['temp_range'][1]:.1f}°C")
...
def display_parcel_summary(df: pd.DataFrame, console: Console):
    ...
    table = Table(title="Per-Parcel Summary", show_header=True, header_style="bold cyan")
    table.add_column("Parcel ID", style="yellow", width=15)
...
def query_sample(
    ...
    try:
        ...
        df = get_random_parcels_in_window(conn, delta_dir, start_date, end_date, num_parcels)
