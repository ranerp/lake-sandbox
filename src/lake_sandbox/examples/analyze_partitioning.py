...
@monitor_performance()
def analyze_partition_distribution(conn: duckdb.DuckDBPyConnection, delta_dir: str) -> dict:
...
def display_partition_analysis(analysis: dict, console: Console):
    ...
    table = Table(title="Partition Distribution Analysis", show_header=True, header_style="bold magenta")
    ...
    # Summary
    summary_table = Table(title="Summary Statistics", show_header=True, header_style="bold cyan")
    ...
    summary_table.add_row("Avg Records/Partition", f"{analysis['total_records'] // analysis['total_partitions']:,}")
    summary_table.add_row("Avg Parcels/Partition", f"{analysis['total_parcels'] // analysis['total_partitions']:,}")
    
    console.print(summary_table)


def display_performance_results(parcel_results: dict, pruning_results: dict, console: Console):
    """Display query performance results."""
    # Parcel query performance
    perf_table = Table(title="Parcel Query Performance", show_header=True, header_style="bold red")
    ...
    pruning_table = Table(title="Partition Pruning Analysis", show_header=True, header_style="bold blue")
...
def analyze_partitioning(
    ...
    try:
        ...
        console.print(f"Selecting {num_test_parcels} random parcels for performance testing...")
        ...
        # Recommendations
        console.print("\n" + "="*60)
        avg_parcel_time = sum(r['query_time'] for r in parcel_results.values()) / len(parcel_results)
