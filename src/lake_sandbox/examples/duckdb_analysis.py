...
def query_parcel_data(conn: duckdb.DuckDBPyConnection, delta_dir: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise ValueError(f"Failed to query Delta table: {e}") from e
...
def analyze_timeseries_stats(df: pd.DataFrame) -> dict:
    ...
    partition_stats = (
        df.groupby("parcel_chunk")
        .agg({"parcel_id": ["count", "nunique"], "ndvi": "mean", "temperature": "mean"})
        .round(3)
    )
    partition_stats.columns = [
        "total_records",
        "unique_parcels",
        "avg_ndvi",
        "avg_temp",
...
def create_vegetation_index_distribution(df: pd.DataFrame):
    ...
    axes[1, 2].bar(
        range(len(partition_counts)), partition_counts.values, alpha=0.7, color="purple"
    )
...
def create_monthly_trends(df: pd.DataFrame):
    ...
    # Temperature trend
    monthly_temp = (
        df_copy.groupby("month_name")["temperature"].mean().reindex(month_order)
    )
    ...
    # Precipitation trend
    monthly_precip = (
        df_copy.groupby("month_name")["precipitation"].mean().reindex(month_order)
    )
    ...
    # Cloud cover trend
    monthly_cloud = (
        df_copy.groupby("month_name")["cloud_cover"].mean().reindex(month_order)
    )
    ...
    monthly_partitions = (
        df_copy.groupby("month_name")["parcel_chunk"].nunique().reindex(month_order)
    )
    axes[1, 2].plot(
        month_order, monthly_partitions, marker="o", linewidth=2, color="purple"
    )
...
def create_partition_analysis(df: pd.DataFrame):
    ...
    partition_counts = df["parcel_chunk"].value_counts().sort_index()
    axes[0, 1].bar(
        range(len(partition_counts.head(20))),
        partition_counts.head(20).values,
        alpha=0.7,
    )
    ...
    axes[1, 0].bar(
        range(len(partition_ndvi)), partition_ndvi.values, alpha=0.7, color="green"
    )
    ...
    axes[1, 1].bar(
        range(len(parcel_counts)), parcel_counts.values, alpha=0.7, color="orange"
    )
...
def analyze_data(
    ...
    sample_parcels: int = typer.Option(
        10, "--sample-parcels", help="Number of parcels to show in timeseries plot"
    ),
    output_dir: str = typer.Option(
        "output/duckdb_analysis",
        "--output-dir",
        help="Directory to save generated plots",
    ),
    show_plots: bool = typer.Option(
        False, "--show-plots", help="Display plots interactively"
    ),
) -> None:
